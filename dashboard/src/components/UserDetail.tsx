        ...
        <>
          ...
          <section className="detail-section">
            ...
            <dl className="detail-grid">
              ...
              <div>
                <dt>1セッション平均リクエスト</dt>
                <dd>{sessions.session_count > 0 ? sessions.avg_requests.toFixed(1) : '-'}</dd>
              </div>
              <div>
                <dt>1セッション平均コスト</dt>
                <dd>{sessions.session_count > 0 ? `$${sessions.avg_cost_usd.toFixed(4)}` : '-'}</dd>
              </div>
              <div>
                <dt>最大コスト/セッション</dt>
                <dd>{sessions.session_count > 0 ? `$${sessions.max_cost_usd.toFixed(4)}` : '-'}</dd>
              </div>
            </dl>
          </section>
              ...
              {terminals.length > 0 && (
                <section className="detail-section">
                  ...
                  <div className="source-grid">
                    {terminals.map((t) => {
                      const pct =
                        totalTerminalReqs > 0 ? (t.request_count / totalTerminalReqs) * 100 : 0;
                ...
                <h4>
                  ...
                  {skills.length > 0 && (
                    <span className="muted">
                      ({skills.length.toLocaleString()} 種 / 全 {totalSkillCalls.toLocaleString()}{' '}
                      回)
                    </span>
                  )}
                </h4>
                  ...
                  <div className="tool-scroll">
                    <table className="detail-table tool-table">
                      ...
                      <tbody>
                        {skills.map((s) => {
                          const pct =
                            totalSkillCalls > 0 ? (s.use_count / totalSkillCalls) * 100 : 0;
